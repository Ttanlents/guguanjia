...
let vm=new Vue({
    ...
    methods: {
        ...
        onClick: function (event, treeId, treeNode) {
            ...
            //绑定node参数的id
            if (treeNode.id!=0&&treeNode.id!=this.officeId) { //如果不是点自己和全部
                //把id和name往回传
                parent.layer.officeId=treeNode.id;
                parent.layer.officeName=treeNode.name;
                parent.layer.success=true;
                let index = parent.layer.getFrameIndex(window.name);
                console.log(index);
                parent.layer.close(index)
            } else {
                layer.msg("请选择正确的区域，不能选机构列表和原来的机构！")
            }
    ...
    created: function () {
        this.officeId=parent.layer.obj;
    },
    mounted: function () {
